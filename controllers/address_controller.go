...
func UserEditAddress(c *fiber.Ctx) error {
	...
	if err := database.DB.First(&add, "id = ? AND user_id = ?", addressID, userID).Error; err != nil{
		if err == gorm.ErrRecordNotFound{
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
				"error": "address not found in the user",
			})
		}
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"error": "failed to retrieve user address",
		})
	}
	...
	if err := database.DB.Model(&NewAddress).Where("id = ? AND user_id = ?", addressID, userID).Updates(&fields).Error; err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
	...
	if err := database.DB.First(&UpdateAddress, "id = ? And user_id = ?", addressID, userID).Error; err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func UserDeleteAddress(c *fiber.Ctx) error {
	...
	if err := database.DB.Where("id = ? AND user_id = ?", addressID, userID).First(&address).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
				"error": "address nof found in user",
			})
		}
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
